...
export class APIPromise<T> extends Promise<T> {
   ...
   * or add one of these imports before your first `import … from 'reductoai'`:
...
const validatePositiveInteger = (name: string, n: unknown): number => {
  if (typeof n !== 'number' || !Number.isInteger(n)) {
    throw new ReductoError(`${name} must be an integer`);
  }
  if (n < 0) {
    throw new ReductoError(`${name} must be a positive integer`);
  }
  return n;
};
...
export const coerceInteger = (value: unknown): number => {
  ...
  throw new ReductoError(`Could not coerce ${value} (type: ${typeof value}) into a number`);
};
...
export function debug(action: string, ...args: any[]) {
  if (typeof process !== 'undefined' && process?.env?.['DEBUG'] === 'true') {
    console.log(`Reducto:DEBUG:${action}`, ...args);
  }
}
...
export const toBase64 = (str: string | null | undefined): string => {
  ...
  throw new ReductoError('Cannot generate b64 string; Expected `Buffer` or `btoa` to be defined');
};
