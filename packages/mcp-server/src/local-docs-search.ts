...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    markdown:
      "## get\n\n`client.job.get(job_id: string): { status: 'Pending' | 'Completed' | 'Failed' | 'Idle'; progress?: number; reason?: string; result?: parse_response | extract_response | split_response | edit_response | pipeline_response | v3_extract | classify_response; } | { status: 'Pending' | 'Completed' | 'Failed' | 'Idle'; bucket?: object; created_at?: string; duration?: number; num_pages?: number; progress?: number; raw_config?: string; reason?: string; result?: parse_response | extract_response | split_response | edit_response | pipeline_response | v3_extract | classify_response; source?: object; total_pages?: number; type?: 'Parse' | 'Extract' | 'Split' | 'Edit' | 'Pipeline' | 'Classify'; }`\n\n**get** `/job/{job_id}`\n\nRetrieve Parse\n\n### Parameters\n\n- `job_id: string`\n\n### Returns\n\n- `{ status: 'Pending' | 'Completed' | 'Failed' | 'Idle'; progress?: number; reason?: string; result?: { duration: number; job_id: string; result: object | object; usage: parse_usage; pdf_url?: string; studio_link?: string; } | object | { result: object | object; usage: parse_usage; } | { document_url: string; form_schema?: edit_widget[]; usage?: parse_usage; } | { job_id: string; result: object; usage: parse_usage; } | object | { job_id: string; result: object; duration?: number; response_confidence?: object; }; } | { status: 'Pending' | 'Completed' | 'Failed' | 'Idle'; bucket?: object; created_at?: string; duration?: number; num_pages?: number; progress?: number; raw_config?: string; reason?: string; result?: { duration: number; job_id: string; result: object | object; usage: parse_usage; pdf_url?: string; studio_link?: string; } | object | { result: object | object; usage: parse_usage; } | { document_url: string; form_schema?: edit_widget[]; usage?: parse_usage; } | { job_id: string; result: object; usage: parse_usage; } | object | { job_id: string; result: object; duration?: number; response_confidence?: object; }; source?: object; total_pages?: number; type?: 'Parse' | 'Extract' | 'Split' | 'Edit' | 'Pipeline' | 'Classify'; }`\n\n### Example\n\n```typescript\nimport Reducto from 'reductoai';\n\nconst client = new Reducto();\n\nconst job = await client.job.get('job_id');\n\nconsole.log(job);\n```",
  },
  {
    name: 'get_all',
    endpoint: '/jobs',
