...
import { ClientOptions } from 'reductoai';

const prompt = `Runs JavaScript code to interact with the Reducto API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    REDUCTO_API_KEY: requireValue(
      readEnv('REDUCTO_API_KEY') ?? client.apiKey,
      'set REDUCTO_API_KEY environment variable or provide apiKey client option',
    ),
    REDUCTO_BASE_URL:
      readEnv('REDUCTO_BASE_URL') ?? readEnv('REDUCTO_ENVIRONMENT') ? undefined : client.baseURL ?? undefined,
  };
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      project_name: 'reductoai',
      code,
      intent,
      client_opts: { environment: (readEnv('REDUCTO_ENVIRONMENT') || undefined) as any },
